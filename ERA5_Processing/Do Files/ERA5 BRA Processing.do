...
*Log
log using "$root\Output\BRA_ERA5_log", replace
...
*Append all days
forvalues year = 2003(1)2019 {
	clear
	display "$S_TIME  $S_DATE"
	set obs 1 // Create Seed Observation
	generate cd_geocmu = . // Create Seed Variable
	...
	forvalues month = 1(1)12 { //MONTH
		...
		forvalues day = 1(1)31 { //DAY
			capture confirm file "$root\GEE Brazil ERA5\BRA_`year'_`month'_`day'.csv"
			if _rc == 0 { //'_rc' is a macro created by 'capture confirm file'
				clear
				import delimited "$root\GEE Brazil ERA5\BRA_`year'_`month'_`day'.csv"
				***Step 2: Adjust
				generate year = `year', after(cd_geocmu)
...
drop if cd_geocmu == .
save "$root\Output\BRA_ERA5.dta", replace
display "$S_TIME  $S_DATE"
